...
variable "cloudSocDataCollectionRolePermissions" {
  ...
  default = [
    ...
    "aiplatform.endpoints.get", "aiplatform.endpoints.list", "bigquery.tables.getIamPolicy"
  ]
}
...
variable "logSinkFilter" {
  ...
  //below logsink filter for filtering only gcs bucket specific control plane and data plane events 
  //((protoPayload.serviceName="storage.googleapis.com" AND (protoPayload.methodName="storage.buckets.create" OR protoPayload.methodName="storage.buckets.delete" OR protoPayload.methodName="storage.buckets.update" OR protoPayload.methodName="storage.setIamPermissions")) OR (protoPayload.methodName="storage.objects.create" OR protoPayload.methodName="storage.objects.delete" OR protoPayload.methodName="storage.objects.update")) AND (severity=INFO OR severity=NOTICE)
}
